...
void HashORACLE(unsigned char* pPlain, int nPlainLen, unsigned char* pHash, string input_salt, int input_saltlen)
{
        char ToEncrypt[256];
        char temp[256];
        char username[256];
        ...
        for (i=1,j=0; j<input_saltlen; i++,j++)
        {
                ToEncrypt[i] = input_salt[j];
                i++;
        }
...
void HashMSCACHE(unsigned char *pPlain, int nPlainLen, unsigned char* pHash, string input_salt, int input_saltlen)
{
	...
	for (i=0; i<input_saltlen; i++)
	{
		unicode_user[i*2] = input_salt[i];
	...
	MD4_Update(&ctx,(unsigned char*) unicode_user,input_saltlen*2);
	MD4_Final(pHash,&ctx);
