...
//
//Additional note, this code was modified from my original john the ripper config generator, so you may see
//a lot of references to JtR in the comments. That's why ;)
...
int addMinLength(ruleType *tempRule);
...
int displaySplash() {
  printf("****************************************************************\n");
  printf("*                 drcrack Config File Generator                *\n");
...
int displayMain() {
  ...
  printf("(2) Set word mangling rules,     (aka add two numbers to the end)\n");
  printf("(3) Save settings\n");
  printf("(4) Load settings\n");
  printf("(5) Quit\n");
...
//////////////////////////////////////////////////////////////
//This is a holdover from the original jtr config file generator
//I haven't deleted it just in case I might make use of it later, but currently nothing calls it
int passRules(choiceType *userChoice) {
  int choice;
  int result;
...
int displayPassMangle() {
  ...
  printf("(3) Change the order in which rules will be run\n");
  printf("(4) Display the current rules\n");
  printf("(5) Go back to the previous menu\n");
...
int displayRules(choiceType* userChoice) {
  ...
  printf("\n-----NOTE, FOR A SLIGHT PERFORMANCE INCREASE IT HELPS TO HAVE RULES WITH THE BIGGEST KEYSPACES FIRST----\n");
  while (tempList!=NULL) {
...
int ruleGuidedMain(choiceType* userChoice) {
  ...
  while ((choice!=6)&&(choice!=7)) {
    ...
    else if (choice==4) { //Add a min length requirement
      result=addMinLength(passRule);
    }
    else if (choice==5) {   //Add Comment
      addCommentRule(tempList); 
    }
    else if (choice==8) {  //Help
      ruleHelp();
    }
...
int displayMainGuide(listType *tempList) {
  ...
  tempRule=tempList->value;
  printf("\n-----------------Welcome to the drcrack rule creation walkthrough-----------------------\n");
  printf("Please Note: All sub-parts of the rule are executed in the order that you enter them\n");
  printf("The exception is any replacement rules which will be applied at the very begining\n");
  ...
  printf("(1) Add a case mangling rule                 - (password -> PASSWORDRD)\n");
  ...
  printf("(4) Add a minimum required length to the word used in this rule\n");
  printf("(5) Add a comment to your rule               - (Comments are good.  You won't regret it)\n");
  printf("(6) Save your rule\n");
  printf("(7) Cancel and go back to the previous menu (WARNING WILL NOT SAVE YOUR RULE!)\n");
  printf("(8) Help and general thoughts on rule creation, (if I get around to writing this)\n");
  choice=getChoice(8);
  return choice;
}
...
int ruleHelp() {
  ...
  printf("    To get a better breakdown of the different rules, check out the talk I gave at Shmoocon 2008, \"Smarter Password Cracking\"\n");
...
int insertMangleRule(ruleType *tempRule) {
  ...
  printf("(3) Add another dictionary word to the end of the password        -(password->passwordtwo)\n");
  printf("(4) duplicate the password                                        -(password12->password12password12)\n");
  printf("(5) Go back to the previous menu\n");
  choice=getChoice(5);
  if (choice==5) {
    return -1;
...
//---Uncomment the following part if I add support for arbitrary letter/character/number insertion
//    else if (choice==3) {
...
//    }
      ...
      strncpy(tempRule->jtrRule,"+",3);
      strncpy(tempRule->desc,"Add another word ",18);
    }
    else if (choice==4) { 
      strncpy(tempRule->jtrRule,"d",3);
      strncpy(tempRule->desc,"Duplicate the password ",30);
    }
...
int caseMangleRule(ruleType *tempRule) {
  ...
  printf("(4) Capitalize the last letter                        -(password->passworD)\n");
  printf("(5) Go back to the previous menu\n");
  choice=getChoice(5); 
  if (choice==5) {
    return -1;
    ...
    if (choice==1) {
      ...
      strncpy(tempRule->desc,"Capitalize the last letter",99);
      strncpy(tempRule->jtrRule,"e",99);
    }
...
int addMinLength(ruleType *tempRule) {
  int choice;
  printf("Please enter in the minimum length for any dicitonary word used in this rule\n");
  choice = getInputValue(0);
  sprintf(tempRule->desc,"Min Dictionary Word Length= %d ",choice);
  sprintf(tempRule->jtrRule,">%d",choice);
}
